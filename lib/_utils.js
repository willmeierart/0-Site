...
export const splitToSpans = (word, styles) => (
  word.split('').map((letter, i) => <span key={i} className='split-span-letter' style={styles}>{ letter }</span>)
)

export const splitTitleToSpans = (word) => (
  word.split('').map((letter, i) => {
    return letter === ' '
    ? <br />
    : <span key={i} className='split-span-letter'>{ letter }</span>
  })
)
...
// really cool one!
// this is how the colors fade from one to the next seamlessly --
// the multiplier of scrollTray-overhang/viewport multiplied against
// distance between the individual color-val columns of a matrix, i.e.:
// [[r1, g1, b1],
//  [r2, g2, b2]]
// and incremented against each [val]1 column
